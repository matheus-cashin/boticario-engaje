...
serve(async (req)=>{
  ...
  try {
    ...
    console.log('🤖 AI Campaign Consultant:', {
      campaignId,
      message: message.substring(0, 100)
    });
    const supabase = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
    // Buscar contexto da campanha
    const { data: schedule } = await supabase.from('schedules').select('*').eq('id', campaignId).single();
    const { data: files } = await supabase.from('campaign_files').select('*').eq('schedule_id', campaignId).order('created_at', {
      ascending: false
    }).limit(5);
    const { data: rule } = await supabase.from('company_rules').select('*').eq('campaign_id', campaignId).eq('status', 'completed').order('created_at', {
      ascending: false
    }).limit(1).single();
    const contextInfo = {
      campaign: schedule,
