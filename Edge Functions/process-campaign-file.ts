...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: fileRecord, error: fetchError } = await supabase.from('campaign_files').select('*').eq('id', fileId).single();
    if (fetchError || !fileRecord) {
      throw new Error(`File not found: ${fetchError?.message}`);
    }
    console.log('📁 File record:', fileRecord.file_name);
    // Update status to processing
    await supabase.from('campaign_files').update({
      status: 'processing'
    }).eq('id', fileId);
    // Download file from storage
    const { data: fileData, error: downloadError } = await supabase.storage.from('campaign-files').download(fileRecord.file_path);
    ...
    const { data: campaignRules, error: rulesError } = await supabase.from('company_rules').select('*').eq('campaign_id', fileRecord.campaign_id).eq('status', 'completed').order('created_at', {
      ascending: false
    }).limit(1);
    if (rulesError) {
    ...
    if (!campaignRules || campaignRules.length === 0) {
      ...
      const { data: rawRules } = await supabase.from('rule_raw').select('*').eq('campaign_id', fileRecord.campaign_id).eq('processing_status', 'completed').order('created_at', {
        ascending: false
      }).limit(1);
    ...
    try {
      const mapperResponse = await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/ai-column-mapper`, {
        method: 'POST',
        headers: {
    ...
    if (!hasValidRules) {
      ...
      const ruleData = campaignRules && campaignRules.length > 0 ? campaignRules[0].rule_json : null;
      if (ruleData) {
        try {
          // Chamar o apurador em background
          const apuradorResponse = await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/ai-apurador`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}`,
              'Content-Type': 'application/json'
